...
- Introduced to content-specific Dashboard pages in the wp-admin Post List.

### Changed

- Moved plugin initialization hooks out of the Parsely class constructor into its own function.
- Devevelopment tool updates:
  Upgraded dev dependencies, Improved Test File organization, Introduced yoast/wp-test-utils, Enforced PHP Coding Standards via CI

### Fixed

- Load the api init script before the tracker so values are populated.
- Encode the current URL in the uuidProfileCall URL
- Improved handling of "home" page edge cases
